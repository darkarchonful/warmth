...
// ---------------------------------------------------------------------------
// C2 — Friday evening nudge. Friday (DOW 5) at local 17:00 (5pm), to both
// partners. Branches on whether the couple already has plans in flight:
//   • Has an open plan — any top-level checklist row not yet 'done' (matches the
//     app's "open plans" concept, the same one the deck gate counts) → nudge
//     them to *finish* it this weekend, deep-linking to Plans.
//   • No open plan at all → the generic "make a plan for the weekend" prompt,
//     deep-linking to the deck.
// Applies to every active, fully-paired couple (no new-couple grace) — a pair
// that just joined still gets the Friday nudge. The two branches use distinct
// types ('weekend_plan' vs 'weekend') so they dedup/telemeter independently; the
// per-local-date dedup_key self-limits each to once a week.
// ---------------------------------------------------------------------------
async function weekendPrompt() {
  const { rows } = await pool.query(`
    ...
    WHERE u.timezone IS NOT NULL
      ...
      AND EXTRACT(HOUR FROM (now() AT TIME ZONE u.timezone)) = 17
  `);
  let sent = 0;
  for (const r of rows) {
    const ok = r.has_open_plan
      ? await dispatch({
          userId: r.id, type: 'weekend_plan', dedupKey: `weekend_plan:${r.localdate}`,
          title: 'Your plans are waiting 💛',
          body: 'Make this the weekend you finally do them.',
          data: { route: '/checklist' },
        })
      : await dispatch({
          userId: r.id, type: 'weekend', dedupKey: `weekend:${r.localdate}`,
          title: "The weekend's almost here 🗓️",
          body: 'Make a plan together for this weekend.',
          data: { route: '/swipe' },
        });
    if (ok) sent++;
  }
